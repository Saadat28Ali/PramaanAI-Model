...
if selfie_file is not None:
    selfie_raw = np.asarray(
        bytearray(selfie_file.read()),
        dtype=np.uint8,
    )
...
if face_result is not None:
    st.markdown(
        '<div class="dn-label">Face Verification</div>',
        unsafe_allow_html=True,
    )

    if face_result.get("error"):
        st.warning(
            f"Face verification could not be completed: "
            f"{face_result.get('message', face_result.get('error'))}"
        )
    else:
        similarity = face_result.get("similarity_score", 0.0)
        match = face_result.get("match", False)
        threshold = face_result.get("threshold", 0.45)

        if match:
            st.success(
                "Face verification successful — the document face "
                "matches the uploaded selfie."
            )
        else:
            st.error(
                "Face verification failed — the document face does "
                "not match the uploaded selfie."
            )
