...
class DocumentRectifier:
    ...
    def rectify(
        ...
        for strategy, detect_fn in [
            ...
            ok, reason = self._validate_corners(
                corners,
                image.shape
            )
            ...
            if not ok:
                logger.debug(
                    f"{strategy} corners rejected: {reason}"
                )
                continue

            rectified = self._apply_perspective_transform(
                image,
                corners
            )

            logger.info(
                f"Rectification succeeded via {strategy} detection"
            )
        ...
        logger.warning(
            ...
            "no valid document boundary detected"
        )
        ...
        return RectificationResult(
            ...
            message=(
                ...
                "Ensure the ID card is fully visible against "
    ...
    def _validate_corners(
        ...
        if avg_w < 1:
            return False, "degenerate width"

        # Normalize so aspect ratio is always >= 1
        ...
        if aspect < self.config.min_aspect_ratio:
            return (
                ...
                (
                    ...
                    f"min {self.config.min_aspect_ratio}"
                ),
            )
        ...
        if aspect > self.config.max_aspect_ratio:
            return (
                ...
                (
                    f"aspect ratio {aspect:.2f} > "
                    f"max {self.config.max_aspect_ratio}"
                ),
            )
        ...
        if max_err > self.config.max_reprojection_error:
            return (
                False,
                (
                    f"reprojection error {max_err:.2f}px "
                    f"> threshold"
                ),
            )
    ...
    def _detect_corners_contour(
        self,
        image: np.ndarray
    ) -> Optional[np.ndarray]:
        """Detect document corners using contours."""
        ...
        min_area = (
            image_area * self.config.min_area_ratio
        )
        ...
        for contour in contours[:5]:
            ...
            approx = cv2.approxPolyDP(
                contour,
                self.config.approx_epsilon_ratio * peri,
                True
            )
    ...
    def _detect_corners_hough(
        ...
        """Fallback corner detection via Hough lines."""
        ...
        # HoughLinesP may return shape (N, 1, 4)
        # or shape (N, 4).
        ...
        # Compute intersections of the four boundary lines
        corners = []
    ...
    def draw_corners(
        ...
        """Draw detected corners and edges on the image."""
